# CURIOSITY ENGINE FOUNDATION & CONTRACTS ENGINE EXECUTION 01 — FREEZE THE PERSONAL-RELEASE PRODUCT CONTRACT + REPOSITORY HARNESS + CANONICAL DOMAIN BOUNDARIES

## Execution Identity

- **Execution:** Curiosity Engine Foundation & Contracts Engine Execution 01
- **Edition:** Personal Terminal Edition V1
- **Wave:** Wave 0 — Blocking
- **Dependencies:** None
- **May run in parallel with:** None until this reaches YES
- **Canonical write scope:** `AGENTS.md`, root project configuration, `src/curiosity/contracts/`, `src/curiosity/config/`, bootstrap docs/tests
- **Prompt version:** Personal Terminal Edition V1 / Execution Pack O1
- **Created:** 3 September 2026
- **Purpose:** Create the repository foundation, durable operating harness, validated canonical contracts, configuration boundaries, and provider-neutral local-first skeleton that every parallel engine can safely consume.
...
Deliver the **Foundation & Contracts Engine** as one coherent, repository-native capability for the Personal Terminal Edition. Every Child Loop must reach `YES`, the integrated engine must preserve shared contracts and local-first/provider-neutral invariants, and fresh final-state verification must support the complete Goal.
...
- Shared contract changes after this execution require serialized contract review and contract-test updates.
- No external service is required for `curiosity --help`, `doctor`, local DB creation, or standalone playback of seeded fixtures.
...
- locate canonical owners relevant to ``AGENTS.md`, root project configuration, `src/curiosity/contracts/`, `src/curiosity/config/`, bootstrap docs/tests`;
...
# CHILD LOOP — TASK 1: Establish Repository Skeleton and Engineering Harness

## Task Source Requirement

The personal release must be reproducible from a clean checkout and executable by any repository-capable coding agent. The repository harness is a product reliability feature, not documentation decoration.

Required outcomes:

- Create/adapt a Python 3.12+ project with `uv`, locked dependencies, src layout, test layout, and an installable `curiosity` CLI entrypoint.
- Create a concise root `AGENTS.md` that records architecture boundaries, canonical verification commands, dependency direction, and non-goals without flooding agent context.
- Configure Ruff and pytest; add deterministic `make`/task/script equivalents only if they simplify cross-platform execution.
- Create `curiosity doctor` skeleton that reports Python/app version, config path, data path, SQLite/FTS5 availability, and provider/source capability state without leaking secrets.
- Ensure Windows/macOS/Linux path handling uses platform-appropriate user data/config directories rather than hard-coded paths.

## Objective

Implement **Establish Repository Skeleton and Engineering Harness** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Establish Repository Skeleton and Engineering Harness** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Fresh environment install from lockfile succeeds.
- `curiosity --help` and `curiosity doctor` execute from the installed entrypoint.
- Ruff and pytest run from documented commands.
- A test proves config/data paths are not hard-coded to one OS/user.
...
# CHILD LOOP — TASK 2: Freeze Canonical Domain Contracts and Truth Layers

## Task Source Requirement

Parallel engines require one shared vocabulary for source records, documents, chunks, knowledge atoms, evidence, cards, profiles, exposures, sessions, and harness events. Contracts must prevent downstream model output from silently becoming source truth.

Required outcomes:

- Implement validated Pydantic domain models corresponding to Shared Architecture Contract canonical records.
- Define stable enums/statuses for source type, claim status, evidence support, card type, job status, session status, and trust/provenance classes.
- Use deterministic/opaque IDs with explicit generation rules; do not use mutable display text as identity.
- Define serialization/version fields and contract tests so later migrations can detect incompatible changes.
- Keep provider payloads out of canonical models except minimized provenance metadata.

## Objective

Implement **Freeze Canonical Domain Contracts and Truth Layers** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Freeze Canonical Domain Contracts and Truth Layers** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Round-trip serialization contract tests for every canonical model.
- Invalid/unknown required values fail validation rather than being silently coerced.
- Tests demonstrate a CuriosityCard cannot masquerade as source evidence.
- Schema snapshots or explicit version assertions detect accidental contract drift.
...
# CHILD LOOP — TASK 3: Create Configuration, Capability, and Provider Boundaries

## Task Source Requirement

The release must work without a mandatory LLM provider and must not import provider-specific logic into core domains.

Required outcomes:

- Define layered config: defaults → user config file → environment overrides → command flags, with deterministic precedence.
- Define capability interfaces/protocols for model generation, embeddings/similarity, source adapters, parsers, clock, and random seed where needed.
- Implement a deterministic `NoLLMProvider` fallback sufficient to keep ingestion, basic atom/card formation, and playback usable.
- Validate secrets by presence/capability without printing secret values.
- Add explicit feature flags for optional YouTube, embeddings/sqlite-vec, and harness integrations.

## Objective

Implement **Create Configuration, Capability, and Provider Boundaries** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Create Configuration, Capability, and Provider Boundaries** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Core tests run with zero external API credentials.
- A fake provider exercises provider contract/error behavior.
- Config precedence tests cover file/environment/CLI conflicts.
- `doctor` identifies unavailable optional capabilities without failing core startup.
...
- Clean checkout → `uv sync` → documented test command → `curiosity doctor` succeeds.
- Contract test suite is green and can be run independently by parallel engines.
- No provider-specific import appears inside canonical contract/domain packages.

# PARENT GOAL GATE

Answer exactly:

**HAVE WE ACHIEVED THE PARENT EXECUTION GOAL FOR FOUNDATION & CONTRACTS ENGINE? — YES / PARTIALLY / NO**
