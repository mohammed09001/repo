# CURIOSITY ENGINE EFFICIENCY & RELIABILITY ENGINE EXECUTION 12 — HARDEN REQUEST BUDGETS + CACHING + BACKPRESSURE + MODEL COST CONTROL + OBSERVABILITY

## Execution Identity

- **Execution:** Curiosity Engine Efficiency & Reliability Engine Execution 12
- **Edition:** Personal Terminal Edition V1
- **Wave:** Wave 5/6 — Cross-cutting audit
- **Dependencies:** Executions 02–06 substantially complete; rerun after 07–11 integration
- **May run in parallel with:** Execution 07/09 completion where write scopes do not conflict
- **Canonical write scope:** Cross-cutting changes only after owner review; diagnostics/benchmarks/tests; avoid creating a parallel framework
- **Prompt version:** Personal Terminal Edition V1 / Execution Pack O1
- **Created:** 3 September 2026
- **Purpose:** Turn the personal release into an efficient, diagnosable system whose cost and latency scale with deliberate refresh work rather than 10-second display frequency.
...
Deliver the **Efficiency & Reliability Engine** as one coherent, repository-native capability for the Personal Terminal Edition. Every Child Loop must reach `YES`, the integrated engine must preserve shared contracts and local-first/provider-neutral invariants, and fresh final-state verification must support the complete Goal.
...
- locate canonical owners relevant to `Cross-cutting changes only after owner review; diagnostics/benchmarks/tests; avoid creating a parallel framework`;
...
# CHILD LOOP — TASK 1: Enforce Network, Parse, and Job Budgets

## Task Source Requirement

Source APIs have rate limits and remote content can be unbounded. Every pipeline run needs explicit resource ceilings and backpressure.

Required outcomes:

- Add centralized budget accounting for source requests, bytes, documents, concurrent fetches, retries, parse work, and job attempts.
- Use exponential backoff with jitter and honor Retry-After where appropriate.
- Use GitHub ETag/conditional requests and Semantic Scholar batch/bulk paths when supported.
- Implement queue backpressure so discovery cannot create an unbounded local job explosion.
- Expose budget usage in structured run summary/doctor diagnostics.

## Objective

Implement **Enforce Network, Parse, and Job Budgets** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Enforce Network, Parse, and Job Budgets** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Synthetic flood discovery cannot exceed configured durable job/request budget.
- 429 fixture honors backoff/Retry-After and does not busy-loop.
- Conditional-fetch fixture demonstrates cache path.
- Batch-capable Semantic Scholar fixture reduces call count vs naive per-paper path.
...
# CHILD LOOP — TASK 2: Enforce Model/Embedding Cost and Context Budgets

## Task Source Requirement

The most avoidable cost is repeated AI work. Deterministic dedupe/filtering must precede model calls and all provider work must be measurable.

Required outcomes:

- Track model calls, input/output token usage when provider reports it, estimated/known cost metadata where configured, and latency.
- Deduplicate document/chunk/atom before model calls.
- Cache model results by prompt-contract version + model + normalized input hash where safe.
- Bound context per call and reject accidental whole-corpus/whole-repository prompts.
- Provide per-run max calls/tokens/cost ceiling with graceful stop/resume.

## Objective

Implement **Enforce Model/Embedding Cost and Context Budgets** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Enforce Model/Embedding Cost and Context Budgets** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Repeated identical job hits cache and avoids second provider call.
- Changed prompt contract/model invalidates relevant cache key.
- Budget ceiling stops further calls and preserves unprocessed jobs.
- Oversized context is chunked/rejected rather than silently sent.
...
# CHILD LOOP — TASK 3: Add Benchmarks, Diagnostics, and Failure Recovery Qualification

## Task Source Requirement

A personal release should be understandable when it breaks. Benchmark the paths that matter without pretending synthetic numbers equal internet scale.

Required outcomes:

- Benchmark local DB lookup, candidate ranking, queue planning, and display tick separately.
- Add structured logs with correlation IDs for run/job/source/document/atom while minimizing sensitive content.
- Add `doctor --deep` or equivalent checks for DB integrity, pending/failed jobs, optional provider reachability, and cache health.
- Add failure injection tests for process kill, database busy, provider timeout, malformed source, and cache corruption.
- Document measured baseline on developer machine as local evidence, not universal SLA.

## Objective

Implement **Add Benchmarks, Diagnostics, and Failure Recovery Qualification** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Add Benchmarks, Diagnostics, and Failure Recovery Qualification** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Display tick benchmark confirms no network/model/parse work.
- Deep doctor identifies seeded failure fixtures.
- Kill/restart integration test preserves idempotency.
- Logs contain identifiers/reasons but not API keys.
...
- 10-second display cadence is economically decoupled from source/model cost.
- Every external/model path has explicit bounded budgets and diagnosable failure behavior.
- Cache/idempotency tests prove unchanged work is reused.

# PARENT GOAL GATE

Answer exactly:

**HAVE WE ACHIEVED THE PARENT EXECUTION GOAL FOR EFFICIENCY & RELIABILITY ENGINE? — YES / PARTIALLY / NO**
