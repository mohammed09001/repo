# CURIOSITY ENGINE END-TO-END INTEGRATION & QUALIFICATION ENGINE EXECUTION 13 — INTEGRATE ALL ENGINES + RUN CLEAN-INSTALL PERSONAL-RELEASE QUALIFICATION + FREEZE V1

## Execution Identity

- **Execution:** Curiosity Engine End-to-End Integration & Qualification Engine Execution 13
- **Edition:** Personal Terminal Edition V1
- **Wave:** Wave 7 — Release Gate
- **Dependencies:** Executions 01–12 = YES or explicitly reconciled
- **May run in parallel with:** None; this is the blocking release gate
- **Canonical write scope:** Whole repository for integration/repair only; preserve engine ownership and avoid feature expansion
- **Prompt version:** Personal Terminal Edition V1 / Execution Pack O1
- **Created:** 3 September 2026
- **Purpose:** Integrate the complete Personal Terminal Edition from source discovery to 10-second terminal playback, remove duplicate/dead paths, and prove the release from a clean local environment.
...
- This execution repairs integration defects but must not introduce new product scope.
- A failing release gate is reported as PARTIALLY/NO; do not lower the bar to obtain V1.
...
- locate canonical owners relevant to `Whole repository for integration/repair only; preserve engine ownership and avoid feature expansion`;
...
# CHILD LOOP — TASK 1: Integrate Canonical End-to-End Data Flow

## Task Source Requirement

The release is one product, not thirteen green unit-test islands. Every engine must connect through canonical contracts with one source of truth per domain.

Required outcomes:

- Trace and exercise source → document → chunk → atom → evidence → verified atom → card → ranking → sequence queue → terminal exposure.
- Verify CLI commands delegate into canonical engine APIs.
- Remove integration-only shims/duplicate models created during parallel work after migrating consumers.
- Re-run migrations/contracts after merge.
- Verify optional harness adapters do not alter core data truth or startup.

## Objective

Implement **Integrate Canonical End-to-End Data Flow** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Integrate Canonical End-to-End Data Flow** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- One fixture-backed E2E test traverses the full canonical data path.
- A real bounded smoke run uses at least one configured live source if credentials/network are available, but release does not depend solely on it.
- Duplicate-owner/residual-reference search is performed.
- Fresh DB and upgraded DB both pass integrated startup.
...
# CHILD LOOP — TASK 2: Qualify the First-Use Personal Workflow

## Task Source Requirement

The user must be able to install and use the release without understanding internal engines.

Required outcomes:

- From clean checkout/install: run init → doctor → configure profile → add/sync source → ingest/build cards → play.
- Provide a deterministic seeded path requiring no paid API for immediate verification.
- Provide a documented richer path using optional provider/source credentials.
- Ensure empty/partial/failure states have actionable terminal messages.
- No commercial/account requirement may appear.

## Objective

Implement **Qualify the First-Use Personal Workflow** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Qualify the First-Use Personal Workflow** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Record exact clean-install commands and outcomes.
- Seeded path produces playable verified fixture cards.
- Optional-provider-disabled path remains usable.
- Restart after first-use workflow preserves profile/content/session state.
...
# CHILD LOOP — TASK 3: Run Adversarial, Privacy, Cost, and Reliability Release Review

## Task Source Requirement

Release qualification must try to break the system across untrusted content, provider failure, budgets, restart, and optional integrations.

Required outcomes:

- Inject malicious ANSI/terminal control content, malformed JSON/HTML/PDF metadata, duplicate sources, rate limits, model invalid output, and stale evidence.
- Verify no prompts/code/model outputs are captured by harness adapters in this release.
- Verify source/model work cannot be triggered by display ticks.
- Verify all documented budgets are effective.
- Inspect dependency licenses/versions and document any pre-v1/optional risk such as sqlite-vec.

## Objective

Implement **Run Adversarial, Privacy, Cost, and Reliability Release Review** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Run Adversarial, Privacy, Cost, and Reliability Release Review** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Adversarial suite passes or blocks release.
- Secret-scan/config review finds no committed keys.
- Network/provider call counters remain zero during isolated playback test.
- Budget exhaustion and recovery path is proven.
...
# CHILD LOOP — TASK 4: Freeze Personal Terminal Edition V1 and Produce Release Report

## Task Source Requirement

A usable personal release needs a stable documented boundary before future commercial work begins.

Required outcomes:

- Freeze contract/schema/app version and create release notes.
- Document supported sources and exact limitations, especially YouTube captions and harness lifecycle visibility.
- Document installation, update, data location, backup/reset, and troubleshooting.
- List intentionally deferred commercial features and frozen source-click behavior.
- Create a concise Commercial Edition Input document containing observed personal-release lessons only; do not design speculative commercial infrastructure in this execution.

## Objective

Implement **Freeze Personal Terminal Edition V1 and Produce Release Report** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Freeze Personal Terminal Edition V1 and Produce Release Report** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Version command reports frozen release version.
- Docs commands match actual CLI help.
- A clean-install reviewer can follow instructions without hidden steps.
- Final full test/lint/contract/E2E suite passes after last release edit.
...
**HAVE WE ACHIEVED THE FINAL GOAL FOR TASK 4? — YES / PARTIALLY / NO**
...
- Clean install and documented first-use workflow succeed.
- Full source-to-terminal E2E path is proven.
- Standalone terminal mode works with no coding-agent adapter.
- No remote/model call occurs on the 10-second display tick.
- At least GitHub + Semantic Scholar + URL/feed paths are release-qualified; YouTube remains metadata/authorized-content only.
- All frozen/out-of-scope commercial/browser behaviors remain absent.
- Final report explicitly states whether this Personal Terminal Edition is ready for the user's daily use.

# PARENT GOAL GATE

Answer exactly:

**HAVE WE ACHIEVED THE PARENT EXECUTION GOAL FOR END-TO-END INTEGRATION & QUALIFICATION ENGINE? — YES / PARTIALLY / NO**
