# CURIOSITY ENGINE CURIOSITY COMPOSITION ENGINE EXECUTION 07 — TURN VERIFIED KNOWLEDGE INTO USEFUL 10-SECOND CURIOSITY CARDS + MINI-SEQUENCES WITHOUT CLICKBAIT

## Execution Identity

- **Execution:** Curiosity Engine Curiosity Composition Engine Execution 07
- **Edition:** Personal Terminal Edition V1
- **Wave:** Wave 5
- **Dependencies:** Execution 05 = YES; Execution 06 = YES
- **May run in parallel with:** Execution 12 audits
- **Canonical write scope:** `src/curiosity/compose/`, composition prompts/rubrics/tests
- **Prompt version:** Personal Terminal Edition V1 / Execution Pack O1
- **Created:** 3 September 2026
- **Purpose:** Create the product's distinctive presentation layer: concise curiosity hooks and explanations that preserve factual meaning, source lineage, and educational value.
...
Deliver the **Curiosity Composition Engine** as one coherent, repository-native capability for the Personal Terminal Edition. Every Child Loop must reach `YES`, the integrated engine must preserve shared contracts and local-first/provider-neutral invariants, and fresh final-state verification must support the complete Goal.
...
- locate canonical owners relevant to ``src/curiosity/compose/`, composition prompts/rubrics/tests`;
...
# CHILD LOOP — TASK 1: Define Card Grammar, Types, and Deterministic Composition Baseline

## Task Source Requirement

A card should fit a roughly 10-second reading window and create a real information gap. It must remain comprehensible without a course context.

Required outcomes:

- Define card types such as `question`, `surprise`, `mechanism`, `comparison`, `history`, `connection`, with stable output limits.
- Define hook/body length budgets and readability constraints.
- Implement deterministic composition from atom claim/explanation as a fallback.
- Always retain atom IDs/source labels internally; presentation cannot become a new truth owner.
- Do not implement clickable-source freeze/resume behavior.

## Objective

Implement **Define Card Grammar, Types, and Deterministic Composition Baseline** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Define Card Grammar, Types, and Deterministic Composition Baseline** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Card schema enforces configured length/required fields.
- Fallback composition works with no LLM.
- Card can trace back to verified atom(s).
- Tests reject empty/sensational unsupported hooks.
...
# CHILD LOOP — TASK 2: Implement Model-Assisted Curiosity Rewriting with Fidelity Guard

## Task Source Requirement

LLM composition may improve curiosity but is allowed to change wording, not factual content. It must not add unsupported entities, numbers, causality, or certainty.

Required outcomes:

- Prompt model with only verified atom content and explicit forbidden additions.
- Generate multiple bounded candidates only when configured; score/select without unbounded Best-of-N cost.
- Run a fidelity comparison between candidate card and underlying verified atoms; reject material additions.
- Record generator model/provenance and cost.
- Include anti-clickbait rules: no fake urgency, no false secret framing, no exaggerated superlatives.

## Objective

Implement **Implement Model-Assisted Curiosity Rewriting with Fidelity Guard** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Implement Model-Assisted Curiosity Rewriting with Fidelity Guard** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Adversarial fake model adding a number/entity not present in atoms is rejected.
- Invalid/overlong candidate is repaired once or rejected according to bounded policy.
- Provider failure falls back to deterministic card if configured.
- Selected card remains traceable to the same evidence as its atoms.
...
# CHILD LOOP — TASK 3: Build Coherent Mini-Sequences

## Task Source Requirement

Not every 10 seconds should jump topics. The engine should support short coherent chains such as hook → mechanism → deeper point → connection while each step remains independently valid.

Required outcomes:

- Create sequence packets of configurable 2–6 cards from related verified atoms.
- Represent relationship reason explicitly: same concept, prerequisite, mechanism, contrast, chronology, or connection.
- Do not fabricate a relationship solely because embeddings are similar.
- Allow single-card items and mixed exploration feed.
- Sequence generation occurs before playback, not inside the tick.

## Objective

Implement **Build Coherent Mini-Sequences** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Build Coherent Mini-Sequences** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Fixture sequence preserves declared relationship reasons.
- Unrelated atoms are not forced into a chain.
- Sequence survives restart and deterministic seed/order tests.
- Playback can consume packet cards without needing composition/model calls.
...
- A verified atom can yield a readable 10-second card in both deterministic and fake-LLM modes.
- Fidelity guard blocks unsupported novelty introduced by composition.
- At least one multi-card coherent sequence is generated from fixture knowledge.

# PARENT GOAL GATE

Answer exactly:

**HAVE WE ACHIEVED THE PARENT EXECUTION GOAL FOR CURIOSITY COMPOSITION ENGINE? — YES / PARTIALLY / NO**
