# CURIOSITY ENGINE LOCAL STORE ENGINE EXECUTION 02 — BUILD THE DURABLE SQLITE KNOWLEDGE STORE + MIGRATIONS + FTS5 + IDEMPOTENT LOCAL STATE

## Execution Identity

- **Execution:** Curiosity Engine Local Store Engine Execution 02
- **Edition:** Personal Terminal Edition V1
- **Wave:** Wave 1 — Parallel
- **Dependencies:** Execution 01 = YES
- **May run in parallel with:** Execution 03 and Execution 10
- **Canonical write scope:** `src/curiosity/store/`, database migrations, store tests
- **Prompt version:** Personal Terminal Edition V1 / Execution Pack O1
- **Created:** 3 September 2026
- **Purpose:** Implement the single-user durable local store that all pipeline stages share, using SQLite as the canonical local database with FTS5 and explicit migrations.
...
Deliver the **Local Store Engine** as one coherent, repository-native capability for the Personal Terminal Edition. Every Child Loop must reach `YES`, the integrated engine must preserve shared contracts and local-first/provider-neutral invariants, and fresh final-state verification must support the complete Goal.
...
- locate canonical owners relevant to ``src/curiosity/store/`, database migrations, store tests`;
...
# CHILD LOOP — TASK 1: Implement Schema, Migrations, and Transaction Boundaries

## Task Source Requirement

One SQLite database should own personal-release durable state: sources, documents, chunks, atoms, evidence, cards, profiles, exposures, sessions, jobs, caches, and adapter state.

Required outcomes:

- Create versioned forward migrations and a schema-version owner.
- Model foreign keys and unique constraints that enforce idempotency/content identity.
- Use WAL mode only if verified safe/appropriate for the local access pattern; configure busy timeout and transactions explicitly.
- Separate mutable processing status from immutable content hashes/provenance.
- Do not create a second database per engine unless evidence proves isolation is necessary.

## Objective

Implement **Implement Schema, Migrations, and Transaction Boundaries** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Implement Schema, Migrations, and Transaction Boundaries** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Fresh DB migration reaches current version.
- Upgrade fixture from at least one prior schema version reaches current version without data loss.
- Rollback/failed-migration behavior leaves a diagnosable state.
- Foreign-key/idempotency constraint tests reject duplicate canonical records.
...
# CHILD LOOP — TASK 2: Implement Repositories, FTS5, and Content-Addressed Cache Metadata

## Task Source Requirement

Interactive ranking/retrieval must be local and fast. SQLite FTS5 is the default full-text capability; vector search is optional.

Required outcomes:

- Implement typed repository/store methods rather than ad-hoc SQL spread across engines.
- Create FTS5 indexes for appropriate searchable text with explicit synchronization strategy.
- Persist content hashes, ETags, Last-Modified, fetch timestamps, parser version, and cache metadata.
- Provide deterministic lookup for duplicate documents/chunks/cards before expensive processing.
- Define optional SimilarityIndex interface without requiring sqlite-vec.

## Objective

Implement **Implement Repositories, FTS5, and Content-Addressed Cache Metadata** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Implement Repositories, FTS5, and Content-Addressed Cache Metadata** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- FTS5 query tests find expected records and do not return stale deleted content.
- Duplicate content hash insertion follows the specified idempotent behavior.
- Store methods remain usable when optional vector capability is unavailable.
- Representative local retrieval latency is measured on a synthetic corpus and documented without claiming production scale.
...
# CHILD LOOP — TASK 3: Implement Durable Jobs, Exposure, and Restart State

## Task Source Requirement

A one-user release does not need Redis. Expensive ingestion/generation should resume safely after process interruption using durable local job state.

Required outcomes:

- Create jobs with idempotency key, stage, status, attempts, next-attempt time, error classification, budget counters, and timestamps.
- Implement lease/claim semantics sufficient to prevent duplicate local workers from processing one job simultaneously.
- Persist sessions/exposures independently of generated card authority.
- Recover abandoned in-progress jobs after a bounded lease timeout.
- Provide store diagnostics/repair inspection without destructive auto-repair.

## Objective

Implement **Implement Durable Jobs, Exposure, and Restart State** as a coherent capability integrated with canonical owners. Preserve the functional intent above while adapting details to verified repository architecture.

## Final Goal for This Child Loop

An independent reviewer can inspect the final repository and fresh evidence and conclude that **Implement Durable Jobs, Exposure, and Restart State** works end-to-end, handles the stated failure boundaries, preserves shared architecture contracts, and leaves no unjustified duplicate ownership or placeholder-only implementation.
...
- Kill/restart simulation proves abandoned jobs can be reclaimed without duplicate durable outputs.
- Two worker simulation cannot both commit the same idempotent result.
- Exposure/session writes do not mutate cards/atoms.
- Corrupt/invalid rows are surfaced diagnostically rather than silently discarded.
...
- All schema/migration/repository tests pass against a temporary fresh SQLite database.
- FTS5 is verified available or doctor reports a clear blocking local capability issue.
- No Redis/PostgreSQL/cloud database dependency exists in the personal release.

# PARENT GOAL GATE

Answer exactly:

**HAVE WE ACHIEVED THE PARENT EXECUTION GOAL FOR LOCAL STORE ENGINE? — YES / PARTIALLY / NO**
